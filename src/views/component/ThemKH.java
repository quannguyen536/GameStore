...
public class ThemKH extends JPanel {
	...
	public ThemKH(JDialog dialog) {
		...
		lblTnKhchHng.setForeground(new Color(255, 204, 0));
		...
		lblTngChi.setForeground(new Color(255, 204, 0));
		...
		lblimTchLy.setForeground(new Color(255, 204, 0));
		...
		lblHngThnhVin.setForeground(new Color(255, 204, 0));
		...
		bt_them.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int diemtichluy = Integer.parseInt(tf_diem.getText());
				int tongchi = Integer.parseInt(tf_tongChi.getText());
				String hang = cb_hangThanhVien.getSelectedItem().toString();
				
				Models_Khachhang khachHang = new Models_Khachhang(0, tenkh, sdt, tongchi, diemtichluy, hang);
				Services.getInstance().getMain().getBody().getKhachhang().themThanhVien(khachHang);
				dialog.dispose();
			}
		});
